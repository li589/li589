...
1. **邮箱**：README.md 末尾「联系我」一节，把 `<!-- 在这里填入你的公开邮箱 -->` 替换为真实邮箱。
2. **数据加载延迟**：统计卡片（github-readme-stats）需要账号有一定提交记录才丰满，刚开通时可能显示“暂无数据”，过一阵自动恢复。
3. **访客计数**：用的是 visitorbadge.io 免费服务，`path` 参数是自定义的，首次访问后才开始计数。
